#!/usr/bin/env python3
"""Captura el 'after' de las pantallas con las mejoras de auditoria aplicadas."""
...
def run(url):
    with sync_playwright() as p:
        ...
        page.evaluate("()=>{const c=window.Alpine.$data(document.querySelector('#app'));c.tab='dash';}")
        page.wait_for_timeout(900); page.screenshot(path=os.path.join(OUT,"a-dash.png")); print("ok a-dash.png")
        page.evaluate("()=>{const c=window.Alpine.$data(document.querySelector('#app'));c.tab='home';c.openNew&&c.openNew();}")
        page.wait_for_timeout(900); page.screenshot(path=os.path.join(OUT,"a-add.png")); print("ok a-add.png")
        br.close()
